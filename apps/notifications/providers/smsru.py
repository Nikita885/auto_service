...
# Коды, которые не исправит повтор: чинить нужно настройки, отправителя
# или номер. Полный список — https://sms.ru/api/status
PERMANENT_CODES = {
    200: "неверный api_id",
    ...
    208: "неправильно указано время отправки",
    209: "номер в стоп-листе",
    210: "используется GET, требуется POST",
    211: "метод не найден",
    212: "текст нужно передавать в UTF-8",
    # Требование операторов, а не конкретного шлюза: с 2023 года отправлять
    # с общего номера нельзя, нужен зарегистрированный буквенный
    # отправитель. Сменой провайдера это не обходится.
    221: "не создан буквенный отправитель — https://sms.ru/?panel=senders",
    230: "превышен лимит сообщений на этот номер за день",
    231: "превышен лимит одинаковых сообщений на этот номер",
    232: "превышен лимит одинаковых сообщений в минуту",
    300: "неверный token",
    301: "неверный пароль или пользователь не найден",
    302: "аккаунт не подтверждён",
}

# 201 «нет денег» формально лечится пополнением счёта, но не за те десять
# минут, что отведены на повторы, — поэтому тоже здесь.
# 220 «сервис временно недоступен» в списке намеренно НЕТ: это как раз тот
# случай, ради которого повторы и существуют.
