"""Контракт SMS-провайдера.

Прикладной код никогда не знает, кто именно шлёт сообщение. Смена шлюза —
это новый класс и одна строка в .env, а не правки в бизнес-логике.
"""

from __future__ import annotations

import abc


class SmsDeliveryError(Exception):
    """Временный сбой: шлюз не ответил, сеть моргнула, кончились деньги.

    Задача Celery повторит отправку с нарастающей паузой.
    """


class SmsRejectedError(Exception):
    """Шлюз отказал навсегда: неверный ключ, чужое имя отправителя,
    несуществующий номер.

    Намеренно не наследник `SmsDeliveryError`: повторять такое бессмысленно,
    и пять ретраев подряд только зашумят логи. Задача пометит уведомление
    как неудачное и остановится.
    """


class SmsProvider(abc.ABC):
    @abc.abstractmethod
    def send(self, phone: str, text: str) -> str:
        """Отправить SMS. Возвращает id сообщения у провайдера."""
        raise NotImplementedError
