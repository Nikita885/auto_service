...
CELERY_BEAT_SCHEDULE = {
    ...
    # Выплата по плечам — «в 00:00 по Челябинску». Задача идёт каждые
    # пять минут и сводит все сутки, которые уже закончились и ещё не
    # сведены. Так выплата приходит в первые минуты после полуночи, а если
    # воркер лежал всю ночь — догоняет пропущенное, по суткам и по порядку.
    "settle-referral-payouts": {
        "task": "apps.referral.tasks.settle_referral_payouts",
        "schedule": 300.0,
    },
}
...
REST_FRAMEWORK = {
    ...
    "DEFAULT_THROTTLE_RATES": {
        ...
        # Проверка кода приглашения открыта без авторизации: по ней
        # можно перебирать шестизначные коды и собирать имена
        # пригласивших. Лимит делает перебор бессмысленным.
        "referral_check": "30/hour",
    },
    # Сколько СВОИХ обратных прокси стоит перед приложением. Значение
    # обязательно: по умолчанию DRF берёт `X-Forwarded-For` целиком, а этот
    # заголовок начинает писать сам клиент — и тогда лимит «10 кодов в час
    # на IP» обходится сменой одной строки в запросе, то есть перебором
    # номеров за чужой счёт. 0 = приложение смотрит в сеть напрямую и
    # доверять нужно только REMOTE_ADDR; на проде за nginx это 1
    # (см. settings/prod.py).
...
OTP = {
    ...
    # ВРЕМЕННАЯ ЗАГЛУШКА. Номера, которым код возвращается в ответе API даже
    # в проде. Нужна, пока не согласован буквенный отправитель у SMS-шлюза:
    # без неё войти в приложение на живом сервере нельзя вообще.
    #
    # Каждый номер здесь — открытая дверь в этот аккаунт: кто угодно
    # запросит код и прочитает его в ответе. Держать список можно только
    # из своих тестовых номеров и только до запуска SMS.
    # `manage.py security_audit` считает непустой список провалом.
    "DEBUG_PHONES": env.list("OTP_DEBUG_PHONES", default=[]),
}
...
# ---------------------------------------------------------------- бэкапы
# Снимки базы кладёт `deploy/backup.sh` — он работает на хосте, мимо Django.
# Приложению эти настройки нужны, чтобы `manage.py security_audit` мог
# сказать, что бэкапов нет или они протухли. Каталог лежит внутри проекта:
# прод монтирует его в контейнер вместе с кодом (`.:/app`).
BACKUPS = {
    "DIR": BASE_DIR / env("BACKUP_DIR", default="backups"),
    # Сколько дней держим снимки. Дамп этой базы весит мегабайты, а места
    # на сервере 2 недели истории не жалко.
    "KEEP_DAYS": env.int("BACKUP_KEEP_DAYS", default=14),
    # Старше скольких часов свежий снимок считается просроченным. Сутки на
    # ежедневный бэкап плюс запас на сдвиг расписания.
    "MAX_AGE_HOURS": env.int("BACKUP_MAX_AGE_HOURS", default=36),
}
...
REFERRAL = {
    ...
    # С какой части чека считаем: "total" — заплаченное деньгами (масло
    # плюс работа минус оплаченное баллами), "work" — только работа. Часть
    # чека, закрытая баллами, новых баллов не рождает: иначе баллы
    # воспроизводили бы сами себя.
    "BASE": env("REFERRAL_BASE", default="total"),
    # Сведение плеч и выплата — раз в сутки, в полночь этого пояса.
    # Asia/Yekaterinburg — это время Челябинска (UTC+5).
    "PAYOUT_TIMEZONE": env("REFERRAL_PAYOUT_TIMEZONE", default="Asia/Yekaterinburg"),
    "PAYOUT_TIMEZONE_LABEL": env("REFERRAL_PAYOUT_TIMEZONE_LABEL", default="по Челябинску"),
...
COMPANY = {
    ...
    # Публичный адрес сайта. Нужен там, где ссылку строит сервер и
    ...
    # Реквизиты в подвале. Нужны не для красоты: операторы связи при
    # регистрации имени отправителя SMS проверяют, что на сайте «при беглом
    # осмотре» видно, чьё это юрлицо или ИП. Без них заявку отклоняют.
    # Пустые значения просто не показываются.
    "LEGAL_NAME": env("COMPANY_LEGAL_NAME", default=""),
    "INN": env("COMPANY_INN", default=""),
    "OGRN": env("COMPANY_OGRN", default=""),
    "LEGAL_ADDRESS": env("COMPANY_LEGAL_ADDRESS", default=""),
}
